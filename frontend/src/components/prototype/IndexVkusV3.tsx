...
export default function IndexVkusV3({ initialRoute, initialParam } = {}) {
  return (
    <ToastProvider>
      <PrototypeV4 initialRoute={initialRoute} initialParam={initialParam} />
    </ToastProvider>
  );
}

function PrototypeV4({ initialRoute, initialParam } = {}) {
  const { push } = useToast();
  ...
  const [route, setRoute] = useState(initialRoute || ROUTES.publicHome);
  const [routeParam, setRouteParam] = useState(initialParam ?? null);
